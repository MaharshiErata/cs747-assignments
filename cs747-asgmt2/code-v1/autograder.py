...
class VerifyOutputPlanner:
    def __init__(self, algorithm, print_error):
        ...
        for algo in algorithm_ls:
            ...
            for in_file in input_file_ls:
                print("\n\n", "-"*100)
                if algo == 'default':
                    cmd_planner = [sys.executable, "planner.py", "--mdp", in_file]
                else:
                    cmd_planner = [sys.executable, "planner.py", "--mdp", in_file, "--algorithm", algo]
        ...
        for in_file in policy_eval_files:
            cmd_planner = [sys.executable, "planner.py", "--mdp", in_file, "--policy", in_file.replace("continuing", "rand-continuing").replace("episodic", "rand-episodic")]
    ...
    def verifyOutput(self, cmd_output, in_file, pe, pol_eval=False):
        ...
        if not len(est) == nstates:
            mistakeFlag = True
            print("\n", "*"*10, "Mistake:Exact number of lines in standard output should be", nstates, "but have", len(est), "*"*10)
...
def run(gridworld, test):
    cmd_encoder = [sys.executable, "encoder.py", "--gridworld", gridworld]
    print("\n", "Generating the MDP encoding using encoder.py")
    with open('verify_attt_mdp', 'w') as f:
        subprocess.call(cmd_encoder, stdout=f)
    cmd_planner = [sys.executable, "planner.py", "--mdp", "verify_attt_mdp"]
    print("\n", "Generating the value policy file using planner.py using default algorithm")
    with open('verify_attt_planner', 'w') as f:
        subprocess.call(cmd_planner, stdout=f)
    cmd_decoder = [sys.executable, "decoder.py", "--mdp", "verify_attt_mdp", "--value-policy", "verify_attt_planner", "--gridworld", test]
